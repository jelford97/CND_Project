...
for i in range(len(havecube)):
    if havecube[i]=='No':
       ...
       #print(pererr)
        ...
        bmin=info[2]
        xcellsize=info[3]    
        ratio=get_ratio(50,inc[i])

        im=dist_ellipse(deltx_pc,50,N1[i],N2[i],xcentre[i],ycentre[i],ratio,PA[i])
...
masserr=data['Mass 50pc']*(totalerr/100)
...
data['Mass 50pc Error']=masserrround
data.to_csv('Python.csv',index=False)
